...
class VideoOptions(private var ctx: Context) {
    ...
    fun cropPic(width: Int, height: Int, x: Int, y: Int, inputPath: String, outputPath: String, outputFileUri: Uri, listener: OnCropVideoListener?, frameCount: Int, cropTime: String) {
        val ff = FFmpeg.getInstance(ctx)
        ff.loadBinary(object : FFmpegLoadBinaryResponseHandler {
            override fun onFinish() {
                Log.e("FFmpegLoad", "onFinish")
